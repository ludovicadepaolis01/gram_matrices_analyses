...
#alexnet
class resnet50_representations(nn.Module):
     model_path = "/leonardo/home/userexternal/ldepaoli/models/resnet50-19c8e357.pth"
     def __init__(self): #images
          super().__init__() #refers to the class that this class inherits from (nn.Modules)
          self.resnet50_pretrained = models.resnet50()
          state_dict = torch.load(resnet50_representations.model_path)
          self.resnet50_pretrained.load_state_dict(state_dict)
          self.resnet50_pretrained.to("cuda")
          self.resnet50_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.resnet50_pretrained.named_modules():
               #print(name)
               #print(module)
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.resnet50_pretrained(images)
