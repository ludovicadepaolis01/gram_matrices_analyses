...
#path = "/leonardo/home/userexternal/ldepaoli/densenet169-b2777c0a.pth"
#state_dict = torch.load(path, map_location="cpu")

#print(type(state_dict))
#print(list(state_dict.keys())[:10])

#densenet169_pretrained = models.densenet169()
#print(densenet169_pretrained)

#inceptionnet
class densenet169_representations(nn.Module):
     model_path = "/leonardo/home/userexternal/ldepaoli/models/densenet169-b2777c0a.pth"
     def __init__(self): #images
          super().__init__() #refers to the class that this class inherits from (nn.Modules)
          self.densenet169_pretrained = models.densenet169()
          state_dict = torch.load(densenet169_representations.model_path)
          new_state_dict = {}
          for k, v in state_dict.items():
               new_key = re.sub(r'\.(\d+)', r'\1', k)
               new_state_dict[new_key] = v
          self.densenet169_pretrained.load_state_dict(new_state_dict)
          self.densenet169_pretrained.to("cuda")
          self.densenet169_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.densenet169_pretrained.named_modules():
               #print(name)
               #print(module)
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.densenet169_pretrained(images)
