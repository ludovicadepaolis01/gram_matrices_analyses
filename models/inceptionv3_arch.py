...
#inceptionv3_pretrained = models.inception_v3()
#print(inceptionv3_pretrained)
...
class inceptionv3_representations(nn.Module):
     ...
     def __init__(self): #images
          ...
          state_dict = torch.load(inceptionv3_representations.model_path)
          self.inceptionv3_pretrained.load_state_dict(state_dict)
          self.inceptionv3_pretrained.AuxLogits = None
          self.inceptionv3_pretrained.to("cuda")
          self.inceptionv3_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.inceptionv3_pretrained.named_modules():
               #print(name)
               #print(module)
               if name.endswith(".bn") and "Mixed" not in name:
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.inceptionv3_pretrained(images)
