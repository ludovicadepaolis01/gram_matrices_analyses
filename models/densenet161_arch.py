...
#path = "/leonardo/home/userexternal/ldepaoli/densenet161-8d451a50.pth"
#state_dict = torch.load(path, map_location="cpu")

#print(type(state_dict))
#print(list(state_dict.keys())[:10])

#densenet161_pretrained = models.densenet161()
#print(densenet161_pretrained)

#inceptionnet
class densenet161_representations(nn.Module):
     model_path = "/leonardo/home/userexternal/ldepaoli/models/densenet161-8d451a50.pth"
     def __init__(self): #images
          super().__init__() #refers to the class that this class inherits from (nn.Modules)
          self.densenet161_pretrained = models.densenet161()
          state_dict = torch.load(densenet161_representations.model_path)
          new_state_dict = {}
          for k, v in state_dict.items():
               new_key = re.sub(r'\.(\d+)', r'\1', k)
               new_state_dict[new_key] = v
          self.densenet161_pretrained.load_state_dict(new_state_dict)
          self.densenet161_pretrained.to("cuda")
          self.densenet161_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.densenet161_pretrained.named_modules():
               #print(name)
               #print(module)
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.densenet161_pretrained(images)
