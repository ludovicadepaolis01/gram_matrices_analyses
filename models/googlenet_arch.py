...
#googlenet_pretrained = models.googlenet()
#print(googlenet_pretrained)

#inceptionnet
class googlenet_representations(nn.Module):
     model_path = "/leonardo/home/userexternal/ldepaoli/models/googlenet-1378be20.pth"
     def __init__(self): #images
          super().__init__() #refers to the class that this class inherits from (nn.Modules)
          self.googlenet_pretrained = models.googlenet()
          state_dict = torch.load(googlenet_representations.model_path)
          self.googlenet_pretrained.load_state_dict(state_dict)
          self.googlenet_pretrained.to("cuda")
          self.googlenet_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.googlenet_pretrained.named_modules():
               #print(name)
               #print(module)
               if name == "conv1.bn" or name == "conv3.bn":
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.googlenet_pretrained(images)
