...
class mobilenet_representations(nn.Module):
     ...
     def __init__(self): #images
          ...
          state_dict = torch.load(mobilenet_representations.model_path)
          self.mobilenet_pretrained.load_state_dict(state_dict)
          self.mobilenet_pretrained.to("cuda")
          self.mobilenet_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.mobilenet_pretrained.named_modules():
               #print(name)
               #print(module)
               if name == "features.0.1" or name.endswith("features.4.conv.0.1") or name.endswith("features.9.conv.0.1") or name.endswith("features.14.conv.0.1") or name == "features.18.1":
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.mobilenet_pretrained(images)
