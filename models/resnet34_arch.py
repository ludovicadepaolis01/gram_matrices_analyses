...
#resnet34_pretrained = models.resnet34()
#print(resnet34_pretrained)

#alexnet
class resnet34_representations(nn.Module):
     model_path = "/leonardo/home/userexternal/ldepaoli/models/resnet34-333f7ec4.pth"
     def __init__(self): #images
          super().__init__() #refers to the class that this class inherits from (nn.Modules)
          self.resnet34_pretrained = models.resnet34()
          state_dict = torch.load(resnet34_representations.model_path)
          self.resnet34_pretrained.load_state_dict(state_dict)
          self.resnet34_pretrained.to("cuda")
          self.resnet34_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.resnet34_pretrained.named_modules():
               #print(name)
               #print(module)
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.resnet34_pretrained(images)
