...
#resnet152_pretrained = models.resnet152()
#print(resnet152_pretrained)
...
class resnet152_representations(nn.Module):
     ...
     def __init__(self): #images
          ...
          state_dict = torch.load(resnet152_representations.model_path)
          self.resnet152_pretrained.load_state_dict(state_dict)
          self.resnet152_pretrained.to("cuda")
          self.resnet152_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.resnet152_pretrained.named_modules():
               #print(name)
               #print(module)
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.resnet152_pretrained(images)
