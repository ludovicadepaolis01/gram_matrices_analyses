...
#squeezenet_pretrained = models.squeezenet1_1()
#print(squeezenet_pretrained)
...
class squeezenet_representations(nn.Module):
     ...
     def __init__(self): #images
          ...
          state_dict = torch.load(squeezenet_representations.model_path)
          self.squeezenet_pretrained.load_state_dict(state_dict)
          self.squeezenet_pretrained.to("cuda")
          self.squeezenet_pretrained.requires_grad_(False) #to not compute gradients with respect to model parameters
          self.hooks = []
          self.feature_maps = {}
          ...
          for (name, module) in self.squeezenet_pretrained.named_modules():
               #print(name)
               #print(module)
               if name == "features.0" or name == "classifier.1":
                    module.name = f"conv_{count}"
     ...
     def forward(self, images):
          ...
          feature_map_n_list = []
          feature_map_list = []
          model = self.squeezenet_pretrained(images)
